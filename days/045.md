...
1. SSH into `stapp03` server, examine file
    ...
    # Examine file
    [banner@stapp03 ~]$ cat /opt/docker/Dockerfile 
    FROM httpd:2.4.43
...
2. Pull `httpd:2.4.43` locally and adjust file to ensure cert filepaths are correct

    ```bash
    # Pull file locally
    [banner@stapp03 docker]$ docker pull httpd:2.4.43

    # Adjust Dockerfile
    [banner@stapp03 ~]$ sudo vi /opt/docker/Dockerfile

    [banner@stapp03 docker]$ cat /opt/docker/Dockerfile 
    FROM httpd:2.4.43
    ...
    # Try to build it
    [banner@stapp03 docker]$ docker build .
    ```

## Key Takeaways
- Got stuck on this one, just try to run it with `docker build .`, see what errors, and make adjustments in sequence
